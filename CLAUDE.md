...
The shared `badge` partial (`shared.liquid`) consumes both: it clips each badge to its content via an `overflow: hidden` wrapper (sized by the layout's `h--`/`w--` class, `aspect-ratio` = content ratio) with the `<img>` scaled (`width: calc(100% / w)`) and `transform: translate`d so only the opaque content shows, plus the invert filter when flagged. Every mark then renders edge-to-edge and hugs the `@`, self-maintaining for any team. `half_vertical` sizes its side-by-side badges by **height** (not width) so trimmed marks stay equal-height and aligned, matching `full`/`half_horizontal`. The wrapper's aspect/width caps are `max-height`, and must stay `max-height` — as an inline `height` they override the `h--N` class and a `var()` that fails to resolve then degrades to `auto`, which un-clips the wrapper and balloons the badge off-screen (archived pape-doc 0088). Full design + rejected alternatives: archived pape-doc 0067.
...
**Separating real traffic from test — use `device`, not the UA bucket.** `emit()` records a `client` UA bucket (`faraday` / `ruby` / `curl` / `browser` / `other`) and a `source` (`test` iff `?test=1`, else `prod`) via pure `lib/client.js`, plus `device` (`trmnl.device.friendly_id`) on `next-game`. ⚠️ **The UA bucket does NOT identify provenance:** TRMNL's real device poller sends a bare `Ruby` UA (the bulk of real traffic) *and* a `Faraday` UA (preview/`team=0`), and our own local `trmnlp`/`curl` share those UAs — so you cannot tell TRMNL from us by UA. (An earlier version assumed `ruby` = local preview and auto-tagged it `test`, which silently buried all real device traffic in the test bucket and led to two wrong conclusions before it was caught — see the `project_telemetry_misclassifies_real_traffic` memory and `pape-docs/0072`.) **The reliable real-install signal is `device != ''`** — a populated friendly_id means a real TRMNL device poll (local `trmnlp` sends the literal `{{...}}`, since trmnlp doesn't interpolate `trmnl.*` into polling URLs; curl sends nothing). For a real-world read filter `blob12 != ''`; `COUNT(DISTINCT blob12)` ≈ unique installs (cross-checks TRMNL's "connections" count). **Append `&test=1` to any manual/dev hit** (curl, browser, `wrangler dev --remote` override) so it's tagged test. Raw `ua` + `device` are logged for later bucket refinement. See `observability-queries.md`; forensic origin in `pape-docs/0075`, the misclassification session that motivated `device` in `pape-docs/0072`.
...
- **Dependabot alerts here are dev-toolchain-only, and the pool's `0.x` range hides the fix.** Every alert this repo has raised came in transitively through the pool/vitest (vite, esbuild, undici, postcss, sharp, `@babel/core`); the sole runtime dep is `upng-js`, so none of it reaches the deployed worker. Because the pool is `0.x`, a `^0.16.x`-style caret **cannot** reach a patched `0.20.x` — so alerts accumulate until the range itself is bumped, and Dependabot files one PR per package instead. Bump the pool first and most of them evaporate at once. Never take `npm audit fix --force` here: it "fixes" undici by *downgrading* the pool to 0.8.71, which is flagged for esbuild, miniflare, and wrangler simultaneously. `package.json` carries an `overrides` floor of `undici: ^7.29.0` (miniflare exact-pins 7.28.0, the top of the advisory range) — keep it a **caret, not `>=`**, which resolves to undici 8.x and jumps a major past what miniflare expects; drop it once miniflare ships a fixed pin.
...
- **Polling URL:** `https://trmnl-sports.carter-pape.workers.dev/next-game?team={{ team_id }}&type={{ game_type }}&locale={{ trmnl.user.locale }}&tz={{ trmnl.user.time_zone_iana }}&device={{ trmnl.device.friendly_id }}` — the `device` param logs the per-device friendly_id for the unique-install count (see Observability). Note `{{ team_id }}` interpolates to the composite *with* a `::Display Name` suffix on real installs (e.g. `135252|4424::Boston Red Sox (MLB)`) — see `pape-docs/0081`.
...
- **Refresh interval:** "Every 15 mins" — but this is a **max-refresh *ceiling*, not a cadence.** TRMNL refreshes on-demand only when a device checks in *and* this instance's screen is the one coming up in the playlist rotation (capped at the ceiling). With a long playlist + overnight sleep, an instance polls the worker only a handful of times/day — far fewer than 96/day. So worker poll volume reflects render events (playlist length × devices × wake schedule), not install count. This is why "missing" teams aren't missing — they just hadn't rotated up. ([on-demand refresh](https://help.trmnl.com/en/articles/15123293-on-demand-plugin-refresh))
